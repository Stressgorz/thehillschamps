...
@section('main-content')
 ...
 <div class="card shadow mb-4">
    ...
    <div class="card-header py-3">
      <h6 class="m-0 font-weight-bold text-primary float-left">Lots Lists</h6>
    </div>
    <div class="card-header py-3">
      <form class="form-horizontal">
          <div class="form-group row">
            ...
            <div class="col-md-3 col-sm-3 col-xs-12">
                <label class="control-label">ib Name</label>
                <div class="form-group">
                    <input type='text' class="form-control" name="ib_name" value="{{ Request::get('ib_name') }}"/>
                </div>
            </div>
            <div class="col-md-3 col-sm-3 col-xs-12">
                <label class="control-label">ib Email</label>
                <div class="form-group">
                    <input type='text' class="form-control" name="ib_email" value="{{ Request::get('ib_email') }}"/>
                </div>
            </div>
          </div>
          <div class="form-group">
    ...
    <div class="card-body">
      <div class="table-responsive">
            ...
            @foreach($lots as $data)
              ...
              @endphp
                <tr>
                    ...
                    <td>{{$data->lots}}</td>
                    <td>{{$data->group}}</td>
                    <td>{{$data->lots_date}}</td>
                    <td>
                    <form method="POST" action="{{route('lot.destroy',[$data->id])}}">
                      @csrf
                      @method('delete')
        ...
        @else
          <h6 class="text-center">No Lots found!!! Please create Lots</h6>
        @endif
      </div>
    </div>
</div>  
