...
<section id="content" class="container pt-0">
        <div class="card one">
            <div class="header d-flex justify-content-between">
                <h3 class="title text-center">LEADERBOARD (IB)</h3>
                @if($data_type == 'team')
                <a href="{{route('admin-get-leaderboard-ib', 'user')}}" class="btn btn-leader">USER</a>
                @elseif($data_type == 'user')
                <a href="{{route('admin-get-leaderboard-ib', 'team')}}" class="btn btn-leader">TEAM</a>
                @endif
            </div>
            <div class="sort">
                <ul class="nav nav-tabs " role="tablist">
                    <li class="nav-item" role="presentation">
                    <a class="btn btn-leaderboard" href="{{route('admin-get-leaderboard-ib', $data_type.'?type=month')}}">Monthly</a>
                    </li>
                    <li class="nav-item" role="presentation">
                    <a class="btn btn-leaderboard" href="{{route('admin-get-leaderboard-ib', $data_type.'?type=year')}}">Yearly</a>
                    </li>
                </ul>
            </div>
            @if($type == 'month')
            <div class="sort">
                <ul class="nav nav-tabs " role="tablist">
                    ...
                    <li class="nav-item" role="presentation">
                    <a class="btn btn-leaderboard" href="{{route('admin-get-leaderboard-ib', $data_type.'?type=month&month='.$month)}}">{{$month}}</a>    
                    </li>
                    @endforeach
                </ul>
            </div>
            ...
            <div class="sort">
                <ul class="nav nav-tabs " role="tablist">
                    ...
                    <li class="nav-item" role="presentation">
                    <a class="btn btn-leaderboard" href="{{route('admin-get-leaderboard-ib', $data_type.'?type=year&year='.$year,)}}">{{$year}}</a>    
                    </li>
                    @endforeach
                </ul>
            </div>
